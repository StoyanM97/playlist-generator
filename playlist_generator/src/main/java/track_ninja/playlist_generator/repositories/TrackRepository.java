...
public interface TrackRepository extends CrudRepository<Track, Long> {
    ...
    @Query(value = "SELECT * FROM tracks t JOIN genres g on t.genre_id = g.genre_id WHERE g.name = ?1 ORDER BY rank LIMIT ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM tracks t JOIN genres g ON t.genre_id = g.genre_id WHERE g.name = ?1 AND artist_id NOT IN ?2 ORDER BY rank LIMIT 1", nativeQuery = true)
    Track findTopTrackByGenreAndArtistNotIn(String genre, Deque<Integer> artistsChecked);

    @Query(value = "SELECT * FROM tracks t JOIN genres g ON t.genre_id = g.genre_id WHERE g.name = ?1 AND track_id NOT IN ?2 ORDER BY rank LIMIT 1", nativeQuery = true)
